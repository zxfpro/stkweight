...
from fastapi import (
    ...
    HTTPException,
    status,
)  # <-- 确保 HTTPException 在这里
from sqlalchemy.exc import IntegrityError  # <-- 确保这一行存在
from stkweight.server.models import User, UserCreate
...
SECRET = (
    "YOUR_SUPER_SECRET_KEY_REPLACE_ME"  # !!! 重要：在生产环境中，请务必使用环境变量
)
...
class UserManager(UUIDIDMixin, BaseUserManager[User, uuid.UUID]):
    ...
    async def create(
        ...
        try:
            ...
            await self.user_db.session.refresh(
                new_user
            )  # 刷新以获取可能由数据库生成的字段（如默认值）
